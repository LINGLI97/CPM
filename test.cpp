...
int main (int argc, char *argv[])
{
    ...
    parser.add<int>("lengthMax", 'R', "the maximal length of text string", false, 1000);
    parser.add<int>("lengthMin", 'L', "the minimal length of text string", false, 100);

    parser.add<int>("round", 'r', "the number of test rounds", false, 1000);
    ...
    for (INT r =0; r< NumRound; r++){
       ...
       // Generate a random length for text_string that is less than max_text_length
        ...
        INT l = (half_diff > 0) ? rand() % (half_diff + 1) : 0;



//        // Hardcoded text string and pattern
//        const char* hardcoded_text = "hkundplyrgzauhxvjtpqycqxjrlajydsizfloslfyngtufogbgwzknyvejvphahrancqhpygcgzzlpfmvbnh";
//        const char* hardcoded_pattern = "h";
//
//        // Get the size of the text and the pattern
//        int text_size = strlen(hardcoded_text);
//        int pattern_size = strlen(hardcoded_pattern);
//        INT l = 0;
//        // Allocate memory for text_string
//        unsigned char* text_string = (unsigned char*)malloc((text_size + 1) * sizeof(unsigned char));
//        if (!text_string) {
//            std::cerr << "Failed to allocate memory for text_string." << std::endl;
//            return EXIT_FAILURE;
//        }
//
//        // Allocate memory for pattern
//        unsigned char* pattern = (unsigned char*)malloc((pattern_size + 1) * sizeof(unsigned char));
//        if (!pattern) {
//            std::cerr << "Failed to allocate memory for pattern." << std::endl;
//            free(text_string); // Clean up previously allocated memory
//            return EXIT_FAILURE;
//        }
//
//        // Copy the hardcoded text and pattern into the allocated memory
//        memcpy(text_string, hardcoded_text, text_size + 1); // +1 to include the null terminator
//        memcpy(pattern, hardcoded_pattern, pattern_size + 1); // +1 to include the null terminator
...
//        if (subInterval.first > -1 and subInterval.second > -1){
        ...
        std::unordered_map<string, vector<INT>> resultsRMQ;
...
//        cout<<" while the length of context l = "<<l<<": "<<endl;
        ...
        for (const auto& interval : subIntervalsRMQNonNeg) {
            ...
            INT start_index = SA[interval.first];
            INT length_of_extraction = pattern_size + 2 * l; // Calculate the total length of the substring
            string extracted_string(reinterpret_cast<char*>(text_string + start_index), length_of_extraction);

            resultsRMQ[extracted_string].push_back(SA[interval.first]);
            for (INT i = interval.first + 1;i < interval.second + 1; i++){

                resultsRMQ[extracted_string].push_back(SA[i]);
        ...
        pair<INT,INT> intervalNaive= pattern_matching (pattern, text_string, SA,  LCP, rmq, n,  pattern_size);

        for (INT i = intervalNaive.first; i < intervalNaive.second + 1; i++){
            INT start = SA[i] - l;
            INT end = SA[i] + pattern_size + l;
            if (start < 0 ){
                continue;
            }
            if (end > n ){
...
//        }
        ...
        if (!areMapsIdentical(resultsRMQ, resultsNaive)){
