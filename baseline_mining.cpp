...
int main (int argc, char *argv[])
{
    ...
    double preConstruction_time = std::chrono::duration_cast < std::chrono::microseconds > (Construction_end - Construction_start).count()*0.000001;
...
//    printstring("SA_rev->string",SA_rev, n,text_string_rev);
    ...
    //Internally mining Algorithm outputs the distinct XPY, for every P of length m with at least tau occurrence in the text
...
//    std::sort(index_array.begin(), index_array.end());
    ...
    for (pair<INT, INT > subInterval:IntervalsPreprocessed){
        ...
        std::vector<std::pair<INT, INT>> subsubIntervals_P;
        ...
        for (pair<INT, INT > interval: IntervalsMapped_mining){
            ...
            for (pair<INT, INT> subsubInterval: subIntervalsScaningMining){
                if (SA[subsubInterval.first] + m +x +y < n +1 ){
                    ...
                    subsubIntervals_P.push_back(subsubInterval);
        ...
        if (subsubIntervals_P.size()>tauminus1){
            cout<<SA[subsubIntervals_P[0].first]<<", "<<SA[subsubIntervals_P[0].second]<<endl;
            cout<<SA[subsubIntervals_P[1].first]<<", "<<SA[subsubIntervals_P[1].second]<<endl;
//            cout<<SA[subsubIntervals_P[2 ].first]<<", "<<SA[subsubIntervals_P[2].second]<<endl;

            cout<<"frq: "<<subsubIntervals_P.size()<<endl;
            count_p++;
        }
    ...
    double Mining_time = std::chrono::duration_cast < std::chrono::microseconds > (Mining_end - Mining_start).count()*0.000001;


    long long memory_used = used_end-used_start;

    cout<<"x = "<<x <<"; y = "<<y<<endl;
    cout<<"There are "<<count_p<< " patterns of the length "<<m<<" whose have more than "<<tau<< " distinct contexts XPY."<<endl;
    cout << "Index memory of IM: " << (double) memory_used / (1024.0 * 1024.0) << " MB" << endl;

    cout<< "Total time for whole mining algorithm internally: "<<preConstruction_time + Mining_time <<endl;
