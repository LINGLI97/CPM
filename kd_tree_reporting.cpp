...
//void KDTreeReporting::rangeSearchIterative(Node* node, const vector<pair<INT, INT>>& ranges, INT &counter) {
////    if (!node) return;
//
//
//
...
KDTreeReporting::KDTreeReporting(vector<Point> &points) {
...
Node* KDTreeReporting::buildIterative(vector<Point> &points) {
...
INT KDTreeReporting::rangeSearch(const vector<pair<INT, INT>>& ranges) {
    INT counter =0;
...
KDTreeReporting::~KDTreeReporting(){
    delete root;
}
//int main() {
//
