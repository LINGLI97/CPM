...
int main (int argc, char *argv[])
{
    ...
    parser.add<int>("Stxxl", 'l', "the usage of Stxxl (Gi)", false, 8);
//    parser.add<int>("Read", 'o', "the usage of other (Gi)", false, 8);
...
//    the size of the block (Mi) B is 2 Mi
    ...
    INT ram_use_stxxl = parser.get<int>("Stxxl");
//    INT ram_use_Read = parser.get<int>("Read");
...
//    unsigned long long int memory_limit_read = ram_use_Read  * 1024;

//
    stream_reader<uint40>* SA =  new stream_reader <uint40> (sa_fname);
    stream_reader<uint40>* LCP =  new stream_reader <uint40> (lcp_fname);
...
//    unsigned long long int memory_limit =ram_use * 1024 * 1024 * 1024 / 4;

    unsigned long long int memory_limit_stxxl =ram_use_stxxl  * 1024 * 1024* 1024;


//    cout<< "memory_limit_read: "<<memory_limit_read<< endl;

    cout<< "memory_limit_stxxl: "<<memory_limit_stxxl<< endl;

    typedef stxxl::sorter<FourInteger, FourIntegerComparatorByi> FourIntegerSorterByi;


    FourIntegerSorterByi four_sorterByi(FourIntegerComparatorByi(), memory_limit_stxxl);
    ...
    stream_reader<uint40>* SA_rev =  new stream_reader <uint40> (sa_fname_reverse);
    stream_reader<uint40>* LCP_rev =  new stream_reader <uint40> (lcp_fname_reverse);
...
//    stream_reader<uint40>* LCP_rev =  new stream_reader <uint40> (lcp_fname_reverse, ram_use/4);
    ...
    TwoIntegerSorter two_sorter(TwoIntegerComparator(),  memory_limit_stxxl);
    ...
    FiveIntegerSorter five_sorter(FiveIntegerComparator(),  memory_limit_stxxl);
...
//    }
    ...
    INT counts = 0; // counter for the number of distinct XPY for each P
    INT count_p = 0; // counter for how many p
    ...
    FourIntegerSorterByl four_sorterByl(FourIntegerComparatorByl(), memory_limit_stxxl);
    ...
    while (!five_sorter.empty()) {
        ...
        if (b == b_pre) {
            // if the third and fourth element are all different
            ...
            if (sb == sb_pre){
                ...
                if (four_sorterByl->i -x > -1 and four_sorterByl->i + m + y -1< text_size) {
                    counts++;
                    StartPosition = four_sorterByl->i;
                }
                ++four_sorterByl;
    ...
    cout<<"Ram usage = "<<ram_use<<" GB"<<endl;

    cout<<"There are "<<count_p<< " patterns whose have more than "<<tau<< " distinct contexts XPY."<<endl;
    ...
    cout<< "Time for SA construction "<<SA_time<<" s"<<endl;
    cout<< "Time for LCP construction "<<LCP_time<<" s"<<endl;
    cout<< "Time for SA_rev construction "<<SA_rev_time<<" s"<<endl;
    cout<< "Time for LCP_rev construction "<<LCP_rev_time<<" s"<<endl;
    cout<< "Time for sorting "<<sorting_time<<" s"<<endl;
    cout<< "Time for whole algorithm "<<SA_time + LCP_time + SA_rev_time + LCP_rev_time + sorting_time<<" s" <<endl;
